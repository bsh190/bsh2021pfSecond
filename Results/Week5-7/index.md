...
#### 3) 접근법의 변화
###### - - 제작한 node를 nodes vector에 담고 node의 자식은 node 구조체에 저장. 이후1 foreach문으로 node->child->mesh->primitive->material 이런순으로 접근하여 드로우 (실패) -
###### - + 제작한 node를 nodes vector에 담고 foreach문으로 하나하나 적용 후 children까지 적용(실패) +
